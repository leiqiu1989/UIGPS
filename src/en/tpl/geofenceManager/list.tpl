<% if(data && data.length >	0) {
    ...
    <tr data-id="<%= item.KeyId %>">
        ...
        <td>
            <a class="td-operator js_list_edit">Edit</a>
            <a class="td-operator js_list_delete">Delete</a>
            <% 
                var status= geofenceStatus(item.Enabled);
                    if(status=='Open'){
            %>
                <a class="td-operator js_list_changeStatus" data-status="0">Close</a>
                <% }else if(status=='Close'){ %>
                    <a class="td-operator js_list_changeStatus" data-status="1">Open</a>
                    <% } %>
        </td>
    </tr>
    <% } } %>